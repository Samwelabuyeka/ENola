...
from ENola.sandbox.sandbox import Sandbox, BackgroundCommand
from ENola.schema import ConfigType
from ENola.utils import find_available_tcp_port
from ENola.exceptions import SandboxInvalidBackgroundCommandError
...
class DockerSSHBox(Sandbox):
    instance_id: str
    container_image: str
    container_name_prefix = 'ENola-sandbox-'
    container_name: str
    ...
    def setup_user(self):
        ...
        # Check if the ENola user exists
        exit_code, logs = self.container.exec_run(
            ['/bin/bash', '-c', 'id -u ENola'],
            workdir=SANDBOX_WORKSPACE_DIR,
        )
        if exit_code == 0:
            # User exists, delete it
            exit_code, logs = self.container.exec_run(
                ['/bin/bash', '-c', 'userdel -r ENola'],
                workdir=SANDBOX_WORKSPACE_DIR,
            )
            if exit_code != 0:
                raise Exception(
                    f'Failed to remove ENola user in sandbox: {logs}')

        if RUN_AS_DEVIN:
            # Create the ENola user
            exit_code, logs = self.container.exec_run(
                ['/bin/bash', '-c',
                 f'useradd -rm -d /home/ENola -s /bin/bash -g root -G sudo -u {USER_ID} ENola'],
                workdir=SANDBOX_WORKSPACE_DIR,
            )
            if exit_code != 0:
                raise Exception(
                    f'Failed to create ENola user in sandbox: {logs}')
            exit_code, logs = self.container.exec_run(
                ['/bin/bash', '-c',
                 f"echo 'ENola:{self._ssh_password}' | chpasswd"],
                workdir=SANDBOX_WORKSPACE_DIR,
            )
            if exit_code != 0:
        ...
        exit_code, logs = self.container.exec_run(
            ['/bin/bash', '-c', "echo 'ENola-sandbox' > /etc/hostname"],
            workdir=SANDBOX_WORKSPACE_DIR,
        )
