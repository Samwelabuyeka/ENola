...
def get_file_handler():
    ...
    file_name = f'ENola_{timestamp}.log'
...
ENola_logger = logging.getLogger('ENola')
ENola_logger.setLevel(logging.INFO)
ENola_logger.addHandler(get_file_handler())
ENola_logger.addHandler(get_console_handler())
ENola_logger.propagate = False
ENola_logger.debug('Logging initialized')
ENola_logger.debug('Logging to %s', os.path.join(
    os.getcwd(), 'logs', 'ENola.log'))
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record):
        ...
        super().emit(record)
        self.stream.close
        ENola_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
