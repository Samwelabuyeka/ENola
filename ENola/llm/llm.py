...
from ENola.logger import llm_prompt_logger, llm_response_logger, ENola_logger
...
class LLM:
    def __init__(self,
        ...
        ENola_logger.info(f'Initializing LLM with model: {model}')
        ...
        def my_wait(retry_state):
            ...
            ENola_logger.warning(f'LLM error: {retry_state.outcome.exception()}')
            ENola_logger.info(f'Attempt #{retry_state.attempt_number} | Sleeping for {seconds}s')
            return seconds
